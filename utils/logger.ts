...
class Logger {
  ...
  /**
   * Función principal para loguear errores
   * @param error - Error object o string
   * @param context - Contexto adicional del error
   * @param errorInfo - Información adicional de React (componentStack)
   */
  logError(error: Error | string, context: LogContext = {}, errorInfo?: ErrorInfo): void {
    ...
    if (errorInfo?.componentStack) {
      logEntry.context.componentStack = errorInfo.componentStack;
    }
...
export const logError = (error: Error | string, context: LogContext = {}, errorInfo?: ErrorInfo): void => {
  logger.logError(error, context, errorInfo);
};
