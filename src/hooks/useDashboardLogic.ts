...
interface DashboardLogicHookReturn extends
  DashboardLogicState,
  DashboardLogicData,
  DashboardLogicActions { }
...
export const useDashboardLogic = (
  ...
  const handleAiAnalysis = useCallback(async (force = false) => {
    if (loadingAi || (isAnalysisDisabled && !force)) return;

    // Verificar si el sistema está degradado
    if (!force && checkSystemDegraded()) {
  ...
  }, [transactions, reservations, retryAttempt, loadingAi, isAnalysisDisabled, startCountdown, failureCount, checkSystemDegraded, activateDegradedMode, isSystemDegraded, degradedUntil]);
